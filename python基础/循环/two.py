...
'''
data0 =log.splitlines()  #按行切割，data0列表存储
data0.pop(0)
# print(data0)
length = len(data0)
types = []
...
for i in range(length):
    data1 = data0[i].split("\t")    #按\t进行切割，data1列表存储
    types.append(data1[0])          #取出文件名（带文件格式后缀）
    sizes.append(data1[1])          #取出文件大小，与文件名一一对应
# print(types)
# print(sizes)
length2 = len(types)
cont_types = []                      #把所有后缀类型收集
for i in range(length2) :
    data3 = types[i].split(".")     #切割出后缀格式
    cont_types.append(data3[1])
# print(cont_types)
# print(sizes)
...
def putRecordToTable(fileType, fileSize):
    for one in fileLenTable:
        if one[0] == fileType:
            one[1] += fileSize       #如果存在该格式的文件，在其原来基础上累加大小
            return
...
for i in range(len(cont_types)):
    putRecordToTable(cont_types[i],int(sizes[i]))

# print(fileLenTable)
for element in fileLenTable :
    print(f"{element[0]:5}\t:{element[1]:10}")
